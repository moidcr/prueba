...
/**
 *	\file       htdocs/core/modules/commande/doc/pdf_custom_order.modules.php
 *	\ingroup    commande
 *	\brief      File of Class to generate PDF orders with template Custom Order
 */
...
/**
 *	Class to generate PDF orders with template Custom_Order
 */
class pdf_custom_order extends ModelePDFCommandes
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "custom_order";
		$this->description = $langs->trans('PDFCustomOrderDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$descPed = $object->lines[$i]->desc;
					$object->lines[$i]->desc = $object->lines[$i]->product_desc;
					$pdf->startTransaction();
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxqty - $this->posxup, 3, $this->posxup, $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxqty - $this->posxup, 4, $this->posxup, $curY, $hideref, $hidedesc);
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
		...
		//exit;
					...
					// Molde
						...
						$pdf->SetXY($curX, $curY);
						$pdf->MultiCell($this->posxtva-10 , 3, $object->lines[$i]->product_ref, 0, 'L');
					// Molde

					// label_product
					$pdf->SetXY($this->posxtva, $curY);
					$pdf->MultiCell($this->posxup-10 , 3, $object->lines[$i]->product_label, 0, 'L');
					// label_product

					$resdesign = $db->query($object->lines[$i]->db->lastquery);
					$fila = $resdesign->fetch_assoc();

					$desgprod = "";
					if(!empty($fila['design']))
					{
						$desgprod = $fila['design'];
					}
					// desgprod Designacion
					$pdf->SetXY($this->posxqty, $curY);
					$pdf->MultiCell($this->posxunit-10 , 3, $desgprod, 0, 'L');
					// desgprod Designacion

					// Quantity
					$qty = pdf_getlineqty($object, $i, $outputlangs, $hidedetails);
					$pdf->SetXY($this->posxunit, $curY);
					$pdf->MultiCell($this->posxdiscount - $this->posxunit - 0.8, 4, $qty, 0, 'R'); // Enough for 6 chars
					// Quantity

					// NumPedido
					$pdf->SetXY($this->posxdiscount, $curY);
					$pdf->MultiCell($this->postotalht - $this->posxdiscount - 0.8, 4, $object->lines[$i]->array_options["options_numped"], 0, 'L'); // Enough for 6 chars
					// NumPedido

					// Ord. Fabric.
					$pdf->SetXY($this->postotalht, $curY);
					$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 4, $object->lines[$i]->array_options["options_ordfabr"], 0, 'L'); // Enough for 6 chars
					// Ord. Fabric.
	...
	{
		// phpcs:enable
		global $conf, $mysoc, $hookmanager, $db;
		...
		$rescontact = $db->query("select concat(t1.firstname, ' ', t1.lastname) as Contact from ".MAIN_DB_PREFIX."socpeople t1 join llx_element_contact t2 on t1.rowid = t2.fk_socpeople where t2.element_id = '$object->id'");
		

		$contacc = $rescontact->fetch_assoc();

		$Segui = "";
		if(!empty($contacc['Contact']))
		{
			$Segui = "<b>".$outputlangs->transnoentities("Deliver").":</b> ".$contacc['Contact'];
		}
		
		//fk_shipping_method
		$resship = $db->query("select t2.libelle from ".MAIN_DB_PREFIX."commande t1 join llx_c_shipment_mode t2 on t1.fk_shipping_method = t2.rowid where t1.rowid = '$object->id'");
		

		$method = $resship->fetch_assoc();
		
		$metho = "";
		if(!empty($method['libelle']))
		{
			$metho = "<b>".$outputlangs->transnoentities("Agency").":</b> ".$method['libelle'];
		}
		...
		if($Segui!="")
		{
			$pdf->writeHTMLCell(190, 3, $col1x, $tab2_top + 0, dol_htmlentitiesbr($Segui), 0, 1);
			$tab2_top = $pdf->getY();
		}
		if($notetoshow)
		{
		...
		$pdf->SetFont('', '', $default_font_size - 1);
		$pdf->writeHTMLCell(190, 3, $col1x, $tab2_top + 0, dol_htmlentitiesbr($notetoshow), 0, 1);
		$tab2_top = $pdf->getY();
		}
		
		if($metho!="")
		{
			$pdf->writeHTMLCell(190, 3, $col1x, $tab2_top + 0, dol_htmlentitiesbr($metho), 0, 1);
			$tab2_top = $pdf->getY();
		}
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '', $outputlangsbis = null)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("Molde"), '', 'L');
		}

		$pdf->line($this->posxtva-1, $tab_top, $this->posxtva-1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxtva - 1, $tab_top + 1);
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("label_product"), '', 'L');
		}

		$pdf->line($this->posxup-1, $tab_top, $this->posxup-1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop))
		{

			$pdf->SetXY($this->posxup - 1, $tab_top + 1);
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("DenominationOr"), '', 'L');
		}

		$pdf->line($this->posxqty-1, $tab_top, $this->posxqty-1, $tab_top + $tab_height); // line takes a position y in 2nd parameter and 4th parameter
		if (empty($hidetop))
		{
			
			$pdf->SetXY($this->posxqty - 1, $tab_top + 1);
			$pdf->MultiCell(108, 2, $outputlangs->transnoentities("DesignationAd"), '', 'L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(100, 2, $outputlangs->transnoentities("QtyOr"), '', 'L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(100, 2, $outputlangs->transnoentities("Nº Pedido"), '', 'L');
		}
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(100, 2, $outputlangs->transnoentities("Ord. Fabric."), '', 'L');
		}
