...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	<td class="nobottom linecoldescription minwidth500imp">
		...
		if (!empty($conf->product->enabled) || !empty($conf->service->enabled))
		{
			...
			{
				$statustoshow = 1;
				$usercanprice = $user->rights->sin_precio->myobject->price;
				$hide_price = 0;
				if(isset($user->rights->sin_precio->myobject->price) && $usercanprice==1){
					$hide_price = 1;
				}
				
				if (!empty($conf->global->ENTREPOT_EXTRA_STATUS)) {
					// hide products in closed warehouse, but show products for internal transfer
					$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, array(), $buyer->id, '1', 0, 'maxwidth500', $hide_price, 'warehouseopen,warehouseinternal', GETPOST('combinations', 'array'));
				} else {
					$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, array(), $buyer->id, '1', 0, 'maxwidth500', $hide_price, '', GETPOST('combinations', 'array'));
				}
	...
	?>
	<td <?php echo $hide_price;?> class="nobottom linecoluht right"><?php $coldisplay++; ?>
