...
/**
 * \file    test/phpunit/Sin_precioFunctionalTest.php
 * \ingroup sin_precio
...
/**
 * Class Sin_precioFunctionalTest
 *
 ...
 *
 * @package Testsin_precio
 */
class Sin_precioFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSin_precio')->click();
		} else {
			// Disable the module
			$this->byHref('modSin_precio')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/sin_precio/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('sin_precio/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/sin_precio/admin/about.php');
		$this->authenticate();
		return $this->assertContains('sin_precio/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('sin_preciowidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSin_precio_Sin_precioTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSin_precio_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
